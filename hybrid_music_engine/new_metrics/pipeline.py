...
def _run_kad(
    ...
    """KAD (MMD-RBF) sobre el mismo extractor que FAD. Devuelve {kad, sigma, ...}."""
    import kad_score

    extractor_kwargs = {"device": device} if extractor_name in {"vggish", "clap", "pann"} else {}
    return kad_score.compute_kad(
        real_folder=real_root,
        gen_folder=generated_root,
        extractor_name=extractor_name,
        extractor_kwargs=extractor_kwargs,
        output_dir=out_dir / "kad",
    )
...
def run_new_metrics(
    ...
    *,
    ...
    # 1b) KAD (MMD-RBF) — complementa a FAD; requiere referencia real.
    if real_root and Path(real_root).exists():
        try:
            report["kad"] = _run_kad(
                Path(real_root), generated_root, out_dir, fad_extractor, device,
            )
        except BaseException as exc:  # noqa: BLE001
            log.warning("KAD falló: %s", exc)
            report["kad"] = {"error": str(exc)}
