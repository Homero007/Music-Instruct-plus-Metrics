...
def frechet_distance(
    ...
    # scipy < 1.18 acepta disp=False (devuelve (sqrt, errest)); scipy >= 1.18 lo
    # eliminó y devuelve solo la matriz. Soportamos ambas versiones.
    sqrt_result = linalg.sqrtm(product)
    sqrt_prod = sqrt_result[0] if isinstance(sqrt_result, tuple) else sqrt_result
...
class PANNExtractor:
    """
    PANN (CNN14) — red entrenada en AudioSet (panns_inference), embedding de 2048
    dimensiones. Sirve como BACKEND DE VALIDACIÓN CRUZADA de FAD frente a VGGish:
    si el ranking de modelos coincide entre vggish y pann, la conclusión es
    robusta al backend de embedding.

    Requiere: pip install panns_inference
    """

    name = "pann"
    dim = 2048

    def __init__(self, device: str = "cpu", sr: int = 32000):
        try:
            import torch  # noqa: F401
            import librosa  # noqa: F401
            from panns_inference import AudioTagging
        except ImportError:
            _dep_error("panns_inference", "pip install panns_inference librosa torch")
        import torch
        import librosa
    ...
    def extract(self, audio_path: Path) -> np.ndarray:
        ...
        audio = y[None, :]  # (1, n_samples)
        # AudioTagging.inference -> (clipwise_output, embedding); embedding (1, 2048)
