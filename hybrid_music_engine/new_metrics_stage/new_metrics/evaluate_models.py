        ...
        for model in models:
            ...
            if "fad" in args.metrics:
                ...
                result = compute_fad(
                    ...
                    extractor_kwargs={"device": args.device} if args.fad_extractor in {"vggish", "clap"} else {},
                    output_dir=fad_out,
...
def save_outputs(scores: pd.DataFrame, ranking: pd.DataFrame, friedman: pd.DataFrame, output_dir: Path) -> dict[str, Path]:
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    with pd.ExcelWriter(workbook_path, engine="openpyxl") as writer:
        ...
        summary = pd.DataFrame({
            ...
            "valor": [
                ...
                "p < 0.05 sugiere diferencias significativas entre modelos.",
            ],
        })
...
def print_math_results(ranking: pd.DataFrame, friedman: pd.DataFrame) -> None:
    ...
    if ranking.empty:
        ...
        cols = [c for c in ["model", "overall_rank", "fad_mean", "clap_score_mean", "tempos_std_mean", "tempo_std_delta_vs_real_mean"] if c in ranking.columns]
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--fad-extractor", choices=["vggish", "clap", "mel"], default="vggish", help="Extractor para FAD.")
