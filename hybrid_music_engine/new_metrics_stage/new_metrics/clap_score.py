...
def compute_clap_score(
    ...
    clap_score = float(np.mean(similarities))
    clap_std = float(np.std(similarities))
    clap_min = float(np.min(similarities))
    clap_max = float(np.max(similarities))

    log.info("CLAP Score (%s) = %.4f  (σ=%.4f, min=%.4f, max=%.4f)",
